...
export default function Roadmap() {
  ...
  return (
    <section className="py-24 md:py-28 bg-white select-none">
      <Container>
        <div className="max-w-[640px] mb-12">
          <p className="text-[12px] font-[650] tracking-widest text-ink-350 uppercase">Roadmap</p>
        ...
        <div className="grid lg:grid-cols-3 gap-8 relative">
          ...
          <div className="hidden lg:block absolute top-[22px] left-8 right-8 h-[2.5px] bg-[#dde4eb] z-0" />
            ...
            return (
              <div
                ...
                className={`relative z-10 rounded-[22px] p-6 border cursor-pointer transition-all duration-300 snap-cursor ${
                  isSelected
                    ? "bg-navy text-white border-navy shadow-lift"
                    : "bg-[#fbfcfe] border-black/[0.07] text-ink-700 hover:border-cyan/40"
                }`}
              >
                <div className="flex items-center justify-between">
                  <div
                    className={`w-[44px] h-[44px] rounded-full text-[12px] font-[700] flex items-center justify-center border-[2.5px] transition-colors ${
                      isSelected
                        ? "bg-cyan border-cyan text-[#032132] shadow-glow"
                        : "bg-white border-[#dde4eb] text-navy"
                    }`}
                  >
                    {i + 1}
                  </div>
                  {isSelected && (
                    <span className="text-[10px] font-bold text-cyan tracking-widest uppercase">
                      ACTIVE FOCUS
                    </span>
                  )}
                </div>
                ...
                <div className="mt-5">
                  <div
                    className={`text-[11.5px] font-[700] tracking-wider uppercase ${isSelected ? "text-cyan" : "text-ink-500"}`}
                  >
                    {ph.p}
                  </div>
                  <div
                    className={`text-[20px] font-[700] mt-1 ${isSelected ? "text-white" : "text-navy"}`}
                  >
                    {ph.t}
                  </div>

                  <ul
                    className={`mt-4 space-y-[9px] text-[14px] ${isSelected ? "text-[#c6d2e2]" : "text-ink-600"}`}
                  >
                    {ph.items.map((it) => (
        ...
        <div className="mt-10 rounded-[28px] border border-black/[0.08] bg-[#fbfcfe] p-6 md:p-8 shadow-soft">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="md:col-span-1">
                <span className="text-[10.5px] font-[750] tracking-widest text-cyan uppercase bg-navy px-3 py-1 rounded-full">
                  PHASE {selectedIdx + 1} METRIC
                </span>
                <div className="text-[22px] font-[750] text-navy mt-4 font-mono">
              ...
              <div className="md:col-span-2 space-y-4">
                <div>
                  ...
                  <p className="text-[14.5px] text-ink-700 mt-1 leading-relaxed">
                ...
                <div className="border-t border-black/[0.06] pt-4">
                  ...
                  <p className="text-[13.5px] text-ink-600 mt-1 leading-relaxed italic">
